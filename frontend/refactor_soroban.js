...
let content = fs.readFileSync('frontend/utils/soroban.ts', 'utf8');
...
// Replace all occurrences of the block with the helper function call
content = content.replace(regex, (match, addressVar) => {
  return `return await signAndSubmitTransaction(kit, txXdr, ${addressVar});`;
});

fs.writeFileSync('frontend/utils/soroban.ts', content, 'utf8');
